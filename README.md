...
- **Performance:** The current implementation prioritizes clarity over speed. Key bottlenecks include: (1) constraint matrices are built row-by-row in Python loops rather than vectorized COO construction, (2) OSQP is re-instantiated every SQP iteration instead of using in-place `update()`, and (3) FK/Jacobian are computed serially per waypoint. For production use, these should be vectorized or ported to C++.
